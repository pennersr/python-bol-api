...
class TransporterCode(Enum):
    ...
    def to_string(cls, transporter_code):
        ...
        assert transporter_code in map(lambda c: c.value, list(TransporterCode))
    ...
    def request(self, method, path="", params={}, data=None):
        ...
        if not uri.startswith("/services"):
            uri = "/services/rest/{group}/{version}{path}".format(
                group=self.group, version=self.api.version, path=path
            )
        ...
        for tag, value in kwargs.items():
            if value is not None:
                ...
                if isinstance(value, dict):
                    text = "\n{}\n{}".format(
                        self._create_request_xml_elements(indent + 1, **value), prefix
                    )
                ...
                if xml:
                    xml += "\n"
                xml += prefix
                xml += "<{tag}>{text}</{tag}>".format(tag=tag, text=text)
        return xml
...
class InvoiceMethods(MethodGroup):
    ...
    def list(self, order_id=None, period_from=None, period_to=None):
        ...
        if period_from or period_to:
            if not isinstance(period_from, date) or not isinstance(period_to, date):
                raise ValueError()
            params["period"] = "/".join(
    ...
    def get(self, invoice_id):
        xml = self.request("GET", "/services/rest/invoices/{}".format(invoice_id))
    ...
    def get_specification(self, invoice_id, page=None):
        ...
        xml = self.request(
            "GET",
            "/services/rest/invoices/{}/specification".format(invoice_id),
            params=params,
        )
...
class PlazaAPI:
    def __init__(self, public_key, private_key, test=False, timeout=None, session=None):
...
x-bol-date:{date}
{uri}""".format(
            content_type=content_type, date=date, method=method, uri=uri
        )
        h = hmac.new(
            self.private_key.encode("utf-8"), msg.encode("utf-8"), hashlib.sha256
        )
